...
static int
aio_get_arrayent_device_params(struct arrayent_cloud *p_arr_cloud)
{
...
static int
aio_get_arrayent_product_params(struct arrayent_cloud *p_arr_cloud)
{
	char product_id[5];
...
int do_cloud_post()
{
	...
	if (ARRAYENT_SUCCESS ==
		...
		if (!strcmp(property_buf,
			    led_control)) {
			if (aio_get_led_state() != property_in_value) {
	...
	sprintf(property_buf, "%d", aio_get_led_state());
...
void aio_cloud_start()
{
	...
	if (WM_SUCCESS != ret &&
		...
		ret = aio_get_arrayent_device_params(p_arr_cloud);
		if (WM_SUCCESS != ret)
			populate_default_device_config(p_arr_cloud);
		ret = aio_get_arrayent_product_params(p_arr_cloud);
		if (WM_SUCCESS != ret)
			populate_default_product_config(p_arr_cloud);
	}
