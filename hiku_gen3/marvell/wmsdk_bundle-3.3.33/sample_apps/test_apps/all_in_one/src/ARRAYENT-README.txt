...
1. Open wm_demo/config.mk file and enable ARRAYENT_CLOUD.
   ...
   To fit this in available memory we need to disable some features in SDK.
     $ cd wmsdk_bundle-x.y.z
     $ make menuconfig
   Disable following:
     a. WPS provisioning support in Application framework
     b. WPS in WLAN.
   Also disable following from wm_demo/config.mk:
     a. APPCONFIG_MDNS_ENABLE
     b. APPCONFIG_WPS_ENABLE
   Now build the application.
2. Flash it using flashprog.
3. Reset the device.
4. If the device is not provisioned, do provisioning using
   either WEBUI or Android / IOS APP.
