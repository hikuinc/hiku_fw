...
static void cloud_handle_cloud(cloud_t *c, struct json_str *jstr,
		   jobj_t *obj, bool *repeat_POST)
{
	char buf[CLOUD_MAX_URL_LEN];
	bool url_requested = false, name_requested = false;

	if (json_get_composite_object(obj, J_NAME_CLOUD)
			== WM_SUCCESS) {
		if (json_get_val_str(obj, J_NAME_URL, buf,
			 CLOUD_MAX_URL_LEN) == WM_SUCCESS) {
			/* Query on cloud URL */
			if (strncmp(buf, QUERY_STR, CLOUD_MAX_URL_LEN)
						== 0) {
				url_requested = true;
			} else {
		...
		if (json_get_val_str(obj, J_NAME_NAME, buf,
			...
			if (strncmp(buf, QUERY_STR, DEVICE_NAME_MAX_LEN)
					== 0) {
				name_requested = true;
			} else {
		  ...
		  jobj_t *obj, bool *repeat_POST)
{
	...
	if (json_get_val_str(obj, J_NAME_DIAG_LIVE, buf, 16) == WM_SUCCESS) {
		if (strncmp(buf, QUERY_STR, 16) == 0) {
			...
			diagnostics_read_stats(jstr, app_psm_hnd);
			json_pop_object(jstr);
			*repeat_POST = true;
		}
	}
	...
	if (json_get_val_str(obj, J_NAME_DIAG_HISTORY, buf, 16) == WM_SUCCESS) {
		if (strncmp(buf, QUERY_STR, 16) == 0) {
			...
			diagnostics_read_stats_psm(jstr, app_psm_hnd);
			json_pop_object(jstr);
			*repeat_POST = true;
		}
	}
		 ...
		 jobj_t *obj, bool *repeat_POST)
{
	...
	if (json_get_composite_object(obj, J_NAME_FIRMWARE)
			== WM_SUCCESS) {
...
{
	if (json_get_composite_object(obj, J_NAME_DATA)
				!= WM_SUCCESS)
		return;
