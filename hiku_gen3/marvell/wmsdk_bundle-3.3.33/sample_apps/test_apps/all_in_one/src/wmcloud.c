...
void cloud_process_cmds(cloud_t *c, jobj_t *obj,
 ...
 * sequence: incremented on every send.
 */
				...
				bool *repeat_POST)
{
	...
	json_str_init(&jstr, c->send_packet, CLOUD_PACKET_MAXSIZE - 1);
	/*
	...
	int ret = json_init(&obj, json_tokens, NUM_TOKENS,  c->recv_packet,
			CLOUD_PACKET_MAXSIZE);
	if (ret != WM_SUCCESS)
		return;
...
static int is_cloud_enabled(bool *enabled)
{
	*enabled = false;
	char str_enabled[2];
	int rv = psm_get_variable_str(app_psm_hnd, CLOUD_ENABLED,
...
static int cloud_init(cloud_t *c)
{
	...
	int status = is_cloud_enabled(&cloud_enabled);
	if (status != WM_SUCCESS) {
...
begin:
	...
	case CLOUD_ACTIVE:
		...
		if (event == EVT_INTERNAL_ERROR)
			sub_state = INTERNAL_ERROR;
...
#if APPCONFIG_HTTPS_CLOUD
int cloud_tls_cert_load()
{
	/* XXX: Setting time to Aug 29 2013. This needs to be fixed. */
	wmtime_time_set_posix(1377778888);

	/* Initialize the TLS client configuration structure */
	tls_client_t tls_cfg;
	memset(&tls_cfg, 0x00, sizeof(tls_client_t));
	tls_cfg.ca_cert = ca_cert;
	tls_cfg.ca_cert_size = sizeof(ca_cert) - 1;
	c.ctx = tls_create_client_context(&tls_cfg);
	if (!c.ctx)
		return -WM_FAIL;
...
void cloud_tls_cert_unload()
{
	if (c.ctx)
		tls_purge_client_context(c.ctx);
}
#endif
