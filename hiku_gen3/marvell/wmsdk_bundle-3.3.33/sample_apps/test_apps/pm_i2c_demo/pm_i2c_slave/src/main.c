 ...
 * of i2c-slave device.
 * Connect a wakeup pin from i2c-master to WAKEUP0 of i2c-slave
 ...
 *
 * Summary:
 * This application demonstrates the use case where the processor enters low
...
static void i2c_slave_demo(os_thread_arg_t data)
{
	...
	while (1) {
		...
		while (time_out > 0) {
			...
			/* If i2c_drv_read is successful before time-out period
		...
		/* check the received command to send appropriate response */
...
static void wk_pin_falling_edge_cb(int pin, void *data)
{
...
int main(void)
{
	...
	pm_mcu_cfg(true, PM2, THRESHOLD_TIME);
