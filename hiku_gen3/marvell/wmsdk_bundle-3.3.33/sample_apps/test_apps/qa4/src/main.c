 ...
 */
/*
 * Application for testing iRobot usecase of uAP and station connection phase
 * in continuous loop.
 *
 * Summary:
 *
 * Application is created to verify uAP and station functionality in
 * continuous loop for long duration in below manner.
 *
 * 1. station connect
 * 2. station disconnect
 * 3. uAP start
 * 4. wlan-scan
 * 5. station connect
 * 6. uAP stop
 * 7. station disconnect
 *
 */
#include <wm_os.h>
...
void event_wlan_init_done(void *data)
{
	...
	strncpy(sta_net.ssid, "74706c696e6b2d74657374", sizeof(sta_net.ssid));
	sta_net.security.type = WLAN_SECURITY_WPA2;
	/* Set the passphrase */
	strncpy(sta_net.security.psk, "w1f14r0b0t$2013",
...
void event_uap_started(void *data)
{
	...
	wifi_send_scan_cmd(BSS_ANY, NULL, NULL, 0, NULL, 0, false);
