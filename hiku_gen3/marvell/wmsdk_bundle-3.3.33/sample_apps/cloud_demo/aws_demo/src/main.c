...
/* Sample Application demonstrating the use of AWS IoT Cloud
 * This demo demonstrates how data can be published on aws cloud using aws
 * shadow APIs. If pushbutton is pressed, led state toggles and the state is
 * published on configured thing.
*/
...
int aws_publish_property_state(ShadowParameters_t *sp)
{
	...
	if (led_requested_state != led_state) {
		...
		ret = aws_iot_shadow_update(&mqtt_client,
					    ...
					    10, true);
		if (ret != WM_SUCCESS) {
			wmprintf("Failed to publish requested state of "
				 "the led\r\n");
			return ret;
		}
		if (led_state)
