...
/*
 * Arrayent Cloud demo application
 *
 * Summary:
 *
...
#define CLOUD_THREAD_PRIORITY 2
#define CLOUD_STACK_SIZE	8192
...
static int arrayent_configure(void)
{
	...
	arrayent_config_t   config  = {
		...
		.aca_thread_priority	    = CLOUD_THREAD_PRIORITY,
		.aca_stack_size		    = CLOUD_STACK_SIZE,
...
static void arrayent_demo_main(os_thread_arg_t data)
{
	...
	do {
		...
		if (pushbutton_a_count_prev != pushbutton_a_count) {
			...
			sprintf(property_out_buf, "%d",
				(int)pushbutton_a_count);
