...
static int offset;
static int http_content_fetch_cb(http_session_t handle __attribute__((unused)),
				 void *content, int len)
{
	wmprintf("Read %d from offset %d. Remaining: %d\r\n", len, offset,
		 mrvl_logo_jpg_len - offset);

	memcpy(content, &mrvl_logo_jpg[offset], len);
...
static void httpc_connect_post_using_callback()
{
	...
	http_req_t req = {
		...
		.content = NULL,
		.content_fetch_cb = http_content_fetch_cb,
		.content_len = mrvl_logo_jpg_len,
	};
	...
	rv = http_add_header(handle, &req, "Content-Type",
			     "image/jpeg");
	if (rv != 0) {
		dbg("Add header failed: %d", rv);
		return;
	}
...
static void httpc_post_url(void *arg)
{
	...
	httpc_connect_post_using_callback();
	print_data();
...
static struct cli_command httpc_post_cmds[] = {
	{"httpc-post-callback", NULL, cmd_httpc_post}
};

/* cli_init function (registration of cli)*/
int httpc_post_cb_cli_init(void)
{
