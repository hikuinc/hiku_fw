...
int wlan_memrdwr_getset(int argc, char *argv[])
{
	...
	ret = wifi_mem_access(action, a2hex_or_atoi(argv[2]),
		&value);

	if (ret == WM_SUCCESS) {
		if (action == ACTION_GET)
			wmprintf("At Memory 0x%x: 0x%x\r\n",
				a2hex_or_atoi(argv[2]), value);
		else
			wmprintf("Set the Memory successfully\r\n");
	} else {
	...
	if (role == WLAN_BSS_ROLE_STA) {
		...
		for (i = 0; i < MAX_IPV6_ADDRESSES; i++) {
			if (addr->ipv6[i].addr_state != IP6_ADDR_INVALID) {
				...
				inet6_ntoa(addr->ipv6[i].address),
				ipv6_addr_state_to_desc(
					addr->ipv6[i].addr_state));
			}
		}
		wmprintf("\r\n");
	}
...
void cmd_wlan_getset_tx_rate_index(int argc, char **argv)
{
	int ret;
	int rate_index;
	...
	if (argc == 3) {
		/* Get the tx-rate index */
		wlan_get_sta_tx_rate_index();
	} else if (argc == 4) {
		/* Set the tx-rate index */
		rate_index = (uint8_t) atoi(argv[3]);
		ret = wlan_set_sta_tx_rate_index(rate_index);
		if (ret != WM_SUCCESS) {
			wlcm_e("Error in setting the tx rate index");
		}
	}
	is_data_rate_auto = wlan_get_current_sta_tx_rate_index(&rate_index);

	if (is_data_rate_auto)
		wmprintf("Date Rate is Auto\r\n");
	else {
		if (rate_index != -1)
			wmprintf("Rate index is :: %d\r\n", rate_index);
		else
			wmprintf("Rate index is not changed\r\n");
	}
}
...
static struct cli_command wlan_pm_commands[] = {
	{"pm-ieeeps-hs-cfg",
...
{
	if (cli_register_commands(wlan_pm_commands,
				  sizeof(wlan_pm_commands) /
				  sizeof(struct cli_command)))
		return WLAN_ERROR_ACTION;

	if (board_sdio_pdn_support())
		if (cli_register_commands(pdn_tests,
					  sizeof(pdn_tests) /
					  sizeof(struct cli_command)))
			return WLAN_ERROR_ACTION;
	return WM_SUCCESS;
