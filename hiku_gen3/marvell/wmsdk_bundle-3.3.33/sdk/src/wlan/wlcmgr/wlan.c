...
/*
 * fixme: This macro redefinition should ideally not be necessary. The
 * network matching code should be in wifi driver. When it is moved later
 * to wifi driver this can be removed.
 */
...
#ifdef CONFIG_HOST_SUPP
	os_timer_t multicast_bad_mic_timer;
	os_timer_t unicast_bad_mic_timer;
	bool multicast_bad_mic:1;
	bool unicast_bad_mic:1;
#endif
				       ...
				       const struct wifi_scan_result *res)
{
	...
	if (network->ssid_specific) {
		if ((!res->ssid_len) ||
		    (memcmp(network->ssid, (char *)res->ssid,
			    res->ssid_len))) {
			wscan_d("ssid mismatch: Got: %s Expected: %s",
	...
	if (network->security_specific) {
		if (!security_profile_matches(&network->security, res)) {
			wscan_d("%s: security profile mismatch",
				network->ssid);
			return WM_SUCCESS;
		}
	}
	...
	if (!wifi_11d_is_channel_allowed(res->Channel)) {
		wscan_d("%d: Channel not allowed.", res->Channel);
		return WM_SUCCESS;
	}

	wscan_d("%s: Match successful", network->ssid);
	return 1;
}
	...
	case WLAN_SECURITY_WPA_WPA2_MIXED:
		...
		if (network->role == WLAN_BSS_ROLE_STA) {
			supplicant_send_cmd(SUPP_STA_ENABLE, NULL,
				...
				NULL, NULL, wlan.mac,
				network->security.is_pmf_required);
			supplicant_send_cmd(SUPP_STA_SESS_INIT, network->ssid,
				0, NULL, NULL, NULL,
				network->bssid, wlan.mac, false);
		}
		break;
#else
...
static void do_scan(struct wlan_network *network)
{
	...
	if (wrapper_wlan_11d_support_is_enabled() &&
		...
		chan_list.scan_type = 1;
		chan_list.scan_time = 120;

		ret = wifi_send_scan_cmd(type, bssid, ssid, 1, &chan_list, 0,
				false);
	}
	if (ret) {
	...
	if (network->role == WLAN_BSS_ROLE_UAP) {
		...
		if (network->security.type == WLAN_SECURITY_WPA2) {
			...
			supplicant_send_cmd(SUPP_UAP_SET_PASSPHRASE,
					...
					NULL, wlan.mac, false);
		} else if (network->security.type == WLAN_SECURITY_NONE) {
			supplicant_send_cmd(SUPP_UAP_SET_PASSPHRASE,
					network->ssid, 0,
					NULL, NULL, NULL,
					NULL, wlan.mac, false);
					   ...
					   enum cm_sta_state *next)
{
	if (wlan.sta_state == CM_STA_SCANNING_USER) {
		...
		*next = wlan.sta_return_to;
		...
		wlcm_d("SM: returned to %s", dbg_sta_state_name(*next));
		wlcm_d("releasing scan lock (connet scan)");
		os_semaphore_put(&wlan.scan_lock);
		wlan.is_scan_lock = 0;
		handle_scan_results();
		*next = wlan.sta_state;
...
static void wlcm_process_association_event(struct wifi_message *msg,
					   enum cm_sta_state *next)
{
#if defined(CONFIG_WPA2_ENTP) || defined(CONFIG_HOST_SUPP)
	int ret;
	struct wlan_network *network = &wlan.networks[wlan.cur_network_idx];
#endif
...
#ifdef CONFIG_HOST_SUPP
static void wlcm_process_mic_error_event(struct wifi_message *msg,
					 enum cm_sta_state *next,
					 struct wlan_network *network)
{
	wlcm_w("Sending deauth because of"
		...
		*next = CM_STA_IDLE;
		...
		if (board_antenna_switch_ctrl()) {
			...
			case 2:
				...
				break;
			case 65535:
				/* Use PinMux 7 on RF_CTRL pins for antenna
				 * diversity */
			      ...
			      enum cm_sta_state *next)
{
	...
	wlan_scan_param = (wlan_scan_params_v2_t *) msg->data;
...
static enum cm_sta_state handle_message(struct wifi_message *msg)
{
	...
	case WIFI_EVENT_HS_CONFIG:
		...
		break;
#ifdef CONFIG_HOST_SUPP
	case WIFI_EVENT_ERR_MULTICAST:
		if (wlan.multicast_bad_mic) {
			os_timer_deactivate(&wlan.multicast_bad_mic_timer);
			wlan.multicast_bad_mic = false;
			wlcm_process_mic_error_event(msg, &next, network);
			wlan.assoc_paused = true;
			os_timer_activate(&wlan.assoc_timer);
		} else {
			wlan.multicast_bad_mic = true;
			os_timer_activate(&wlan.multicast_bad_mic_timer);
		}
		break;
	case WIFI_EVENT_ERR_UNICAST:
		if (wlan.unicast_bad_mic) {
			os_timer_deactivate(&wlan.unicast_bad_mic_timer);
			wlan.unicast_bad_mic = false;
			wlcm_process_mic_error_event(msg, &next, network);
			wlan.assoc_paused = true;
			os_timer_activate(&wlan.assoc_timer);
		} else {
			wlan.unicast_bad_mic = true;
			os_timer_activate(&wlan.unicast_bad_mic_timer);
		}
		break;
#endif
#ifdef CONFIG_11N
...
static int wlcm_supp_callback(enum supp_event event, void *data, uint16_t len)
{
	...
	if (event == SUPP_SESSION_FAILED) {
		ret = wlan_wlcmgr_send_msg(WIFI_EVENT_AUTHENTICATION,
				WIFI_EVENT_REASON_FAILURE,
				(void *) IEEEtypes_REASON_4WAY_HANDSHK_TIMEOUT);
	}
	return ret;
}
#endif
...
#ifdef CONFIG_HOST_SUPP
static void multicast_bad_mic_timer_cb(os_timer_arg_t arg)
{
	wlan.multicast_bad_mic = false;
}

static void unicast_bad_mic_timer_cb(os_timer_arg_t arg)
{
	wlan.unicast_bad_mic = false;
}
#endif
...
int wlan_start(int (*cb) (enum wlan_event_reason reason, void *data))
{
	...
	ret = os_timer_create(&wlan.multicast_bad_mic_timer,
			"multicast-bad-mic-timer",
			os_msec_to_ticks(BAD_MIC_TIMEOUT),
			&multicast_bad_mic_timer_cb,
			NULL,
			OS_TIMER_ONE_SHOT,
			OS_TIMER_NO_ACTIVATE);
	if (ret != WM_SUCCESS) {
		wlcm_e("Unable to start multicast bad mic timer");
		return ret;
	}

	ret = os_timer_create(&wlan.unicast_bad_mic_timer,
			"unicast-bad-mic-timer",
			os_msec_to_ticks(BAD_MIC_TIMEOUT),
			&unicast_bad_mic_timer_cb,
			NULL,
			OS_TIMER_ONE_SHOT,
			OS_TIMER_NO_ACTIVATE);
	if (ret != WM_SUCCESS) {
...
#ifdef CONFIG_HOST_SUPP
	supplicant_send_cmd(SUPP_STA_FREE, NULL, 0, NULL,
			NULL, NULL, NULL, wlan.mac, false);
#endif
...
#ifdef CONFIG_HOST_SUPP
	ret = os_timer_delete(&wlan.multicast_bad_mic_timer);
	if (ret != WM_SUCCESS) {
		wlcm_w("failed to delete multicast bad mic timer: %d.", ret);
		return WLAN_ERROR_STATE;
	}
	ret = os_timer_delete(&wlan.unicast_bad_mic_timer);
	if (ret != WM_SUCCESS) {
		wlcm_w("failed to delete multicast bad mic timer: %d.", ret);
		return WLAN_ERROR_STATE;
	}
...
{
	bzero(net, sizeof(struct wlan_network));
	/* Set profile name */
...
int wlan_add_network(struct wlan_network *network)
{
	...
	if ((network->role == WLAN_BSS_ROLE_STA) &&
		...
		supplicant_send_cmd(SUPP_STA_SET_PASSPHRASE, network->ssid,
			0, NULL, NULL, network->security.psk,
			NULL, wlan.mac, false);
	}
#else
	...
	for (i = 0; i < ARRAY_SIZE(wlan.networks); i++) {
		if (wlan.networks[i].name[0] != '\0' &&
			...
			if ((wlan.networks[i].role == WLAN_BSS_ROLE_STA) &&
				...
				supplicant_send_cmd(SUPP_STA_REMOVE_PASSPHRASE,
					wlan.networks[i].ssid,
					0, NULL, NULL, NULL,
					NULL, wlan.mac, false);
			}
#endif
...
int wlan_connect(char *name)
{
	...
	for (i = 0; i < ARRAY_SIZE(wlan.networks); i++)
		if (wlan.networks[i].name[0] != '\0' &&
			...
			wlcm_d("taking the scan lock (connet scan)");
			dbg_lock_info();
...
int wlan_scan(int (*cb) (unsigned int count))
{
	...
	return wlan_scan_with_opt(wlan_scan_param);
}
...
int wlan_scan_with_opt(wlan_scan_params_v2_t *wlan_scan_param)
{
	...
	return send_user_request(CM_STA_USER_REQUEST_SCAN,
			(int)wlan_scan_param);
}
...
int wlan_inject_frame(const uint8_t *buff, const size_t len)
{
	return raw_low_level_output(BSS_TYPE_STA, buff, len);
}
				...
				const uint32_t duration)
{
	...
	return wifi_send_remain_on_channel_cmd(&roc);
}
...
{
	return wifi_set_rx_mgmt_indication(mgmt_subtype_mask);
}
