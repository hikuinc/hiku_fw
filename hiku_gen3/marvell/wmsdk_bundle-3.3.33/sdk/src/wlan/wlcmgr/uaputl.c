...
static void dump_uaputl_usage()
{
	...
	wmprintf("\r\nTo get/set uAP STA ageout value\r\n");
	wmprintf("uaputl sys_cfg_sta_ageout_timer [STA_AGEOUT_TIMER]\r\n");
	wmprintf("\r\nTo get/set uAP PS STA ageout value\r\n");
	wmprintf("uaputl sys_cfg_ps_sta_ageout_timer "
			"[PS_STA_AGEOUT_TIMER]\r\n");
	wmprintf("\r\nTo get/set uAP group re-key time interval"
				", in seconds\r\n");
	wmprintf("uaputl sys_cfg_group_rekey_timer [GROUP_REKEY_TIMER]\r\n");
	wmprintf("\r\nTo get/set uAP custom IEs for management frames\r\n");
	wmprintf("uaputl sys_cfg_custom_ie <TYPE> [INDEX] [IEBuffer]\r\n");
	wmprintf("TYPE: 1 - Clear custom IE at given INDEX\r\n");
	wmprintf("      2 - Get custom IE at given INDEX\r\n");
	wmprintf("      3 - Set custom IE from given IEBuffer\r\n");
	wmprintf("\r\nTo get/set uAP rates:\r\n");
	wmprintf("uaputl sys_cfg_rates <rates>\r\n");
	wmprintf("eg: uaputl sys_cfg_rates 0x82 0x84 0x8b 0x96 "
			"0x0c 0x12 0x18 0x30\r\n\r\n");
	wmprintf("uaputl sys_cfg_mcbc_data_rate <rate>\r\n");
	wmprintf("eg: uaputl sys_cfg_mcbc_data_rate 22\r\n"
...
static void uaputl_mcbc_rate_getset(int argc, char **argv)
{
	...
	if (argc == 2) {
		ret = wifi_uap_mcbc_rate_getset(ACTION_GET, &mcbc_rate);
		if (ret == WM_SUCCESS) {
			if (!mcbc_rate)
				wmprintf("MCBC Data Rate = Auto\r\n");
			else
				wmprintf("MCBC Data Rate = %d.%d M\r\n",
				mcbc_rate / 2, mcbc_rate % 2 == 0 ? 0 : 5);
		} else
			wlcm_e("Could not get MCBC Data rate!\r\n");
	} else {
		mcbc_rate = (t_u16) a2hex_or_atoi(argv[i + 2]);
		ret = wifi_uap_mcbc_rate_getset(ACTION_SET,
				&mcbc_rate);
		if (ret == WM_SUCCESS)
			wmprintf("MCBC Rate setting successful\r\n");
		else
			wlcm_e("Could not set MCBC Data rate!");
	}
}
...
static void uaputl_sta_ageout_timer_getset(int argc, char *argv[])
{
	uint32_t sta_ageout_timer = 0;
	int ret;

	if (argc == 2) {
		ret = wifi_uap_sta_ageout_timer_getset(ACTION_GET,
					&sta_ageout_timer);
		if (ret == WM_SUCCESS)
			wmprintf("STA Ageout Timer = %d \r\n",
					sta_ageout_timer);
		else
			wlcm_e("Could not get STA Ageout Timer!");
	} else {
		sta_ageout_timer = (uint32_t) atoi(argv[2]);
		ret = wifi_uap_sta_ageout_timer_getset(ACTION_SET,
					&sta_ageout_timer);
		if (ret == WM_SUCCESS)
			wmprintf("STA Ageout Timer setting successful\r\n");
		else
			wlcm_e("Could not set STA Ageout Timer!");
	}
}

static void uaputl_ps_sta_ageout_timer_getset(int argc, char *argv[])
{
	uint32_t ps_sta_ageout_timer = 0;
	int ret;

	if (argc == 2) {
		ret = wifi_uap_ps_sta_ageout_timer_getset(ACTION_GET,
					&ps_sta_ageout_timer);
		if (ret == WM_SUCCESS)
			wmprintf("PS STA Ageout Timer = %d \r\n",
					ps_sta_ageout_timer);
		else
			wlcm_e("Could not get PS STA Ageout Timer!");
	} else {
		ps_sta_ageout_timer = (uint32_t) atoi(argv[2]);
		ret = wifi_uap_ps_sta_ageout_timer_getset(ACTION_SET,
					&ps_sta_ageout_timer);
		if (ret == WM_SUCCESS)
			wmprintf("PS STA Ageout Timer setting successful\r\n");
		else
			wlcm_e("Could not set PS STA Ageout Timer!");
	}
}

static void uaputl_group_rekey_timer_getset(int argc, char *argv[])
{
	uint32_t group_rekey_timer = 0;
	int ret;

	if (argc == 2) {
		ret = wifi_uap_group_rekey_timer_getset(ACTION_GET,
					&group_rekey_timer);
		if (ret == WM_SUCCESS)
			wmprintf("Group Rekey Time = %d seconds\r\n",
					group_rekey_timer);
		else
			wlcm_e("Could not get Group Rekey Time!");
	} else {
		group_rekey_timer = (uint32_t) atoi(argv[2]);
		ret = wifi_uap_group_rekey_timer_getset(ACTION_SET,
					&group_rekey_timer);
		if (ret == WM_SUCCESS)
			wmprintf("Group Rekey Time setting successful\r\n");
		else
			wlcm_e("Could not set Group Rekey Time!");
	}
}

static void uaputl_custom_ie_getset(int argc, char *argv[])
{
	uint8_t type, index, buf[MAX_IE_BUFFER_LEN];
	int ret, id;
	unsigned int buf_len, ie_buf_len, ie_len;

	if (argc == 4) {
		type = (uint8_t) atoi(argv[2]);
		if (type <= 0 || type >= 4) {
			wlcm_e("Invalid type\r\n");
			return;
		}
		...
		if (type == 1 || type == 2) {
			...
			if (index < 0 || index > 3) {
				wlcm_e("Invalid index\r\n");
				return;
			}

			if (type == 1) {
				ret = wlan_clear_mgmt_ie(WLAN_BSS_TYPE_UAP,
						index);
				if (ret == WM_SUCCESS)
					wmprintf("Custom IE clear "
							"successful\r\n");
				else
					wlcm_e("Could not clean custom IE!");
			}
			if (type == 2) {
				ret = wlan_get_mgmt_ie(WLAN_BSS_TYPE_UAP,
						index, buf, &buf_len);
				if (ret == WM_SUCCESS) {
					wmprintf("Custom IE get "
							"successful\r\n");
					if (buf_len)
					...
					wlcm_e("Could not get custom IE!");
			}
		} else if (type == 3) {
			if (!ishexstring(argv[3])) {
				wlcm_e("Only hex digits are allowed");
				return;
			}
			...
			if (ie_len > MAX_IE_BUFFER_LEN) {
				wlcm_e("Incorrect IE length %d", ie_buf_len);
				return;
			}

			hex2bin((const uint8_t *)argv[3], buf, ie_len);

			id = (unsigned int) buf[0];
			ie_len = (unsigned int) buf[1];

			memcpy(buf, buf + 2, ie_len);

			ret = wlan_set_mgmt_ie(WLAN_BSS_TYPE_UAP,
						id, buf, ie_len);

			if (ret < 0)
				wlcm_e("Could not set custom IE!");
			else
				wmprintf("Custom IE set at "
						"index :: %d\r\n", ret);
		}
	} else {
...
{
	if (is_uap_started()) {
		wlcm_e("%s failed, as the uAP is already running",
				argv[1]);
		return;
	}
	...
	uap_config.ip.ipv4.addr_type = 0;
	uap_config.ssid_specific = 1;

	/* IP Addr - 192.168.10.1 */
	uap_config.ip.ipv4.address = htonl(0xc0a80a01UL);
	/* Gateway - 192.168.10.1 */
	uap_config.ip.ipv4.gw = htonl(0xc0a80a01UL);
	/* Netmask - 255.255.255.0 */
	uap_config.ip.ipv4.netmask = htonl(0xffffff00UL);
...
static void uaputl_get_param_values(int argc, char **argv)
{
	...
	if (!strcmp(argv[1], "sys_cfg_sta_ageout_timer")) {
		uaputl_sta_ageout_timer_getset(argc, argv);
		return;
	}

	if (!strcmp(argv[1], "sys_cfg_ps_sta_ageout_timer")) {
		uaputl_ps_sta_ageout_timer_getset(argc, argv);
		return;
	}
	if (!strcmp(argv[1], "sys_cfg_group_rekey_timer")) {
		uaputl_group_rekey_timer_getset(argc, argv);
		return;
	}
	if (!strcmp(argv[1], "sys_cfg_custom_ie")) {
		uaputl_custom_ie_getset(argc, argv);
		return;
	}
	...
	if (!strcmp(argv[1], "sys_cfg_mcbc_data_rate")) {
		uaputl_mcbc_rate_getset(argc, argv);
		return;
	}
...
static void uaputl_set_param_values(int argc, char **argv)
{
	...
	if (strcmp(argv[1], "sys_cfg_rates")
		&& strcmp(argv[1], "regrdwr")
		&& strcmp(argv[1], "sys_cfg_custom_ie") && argc != 3)
		goto done;

	if (is_uap_started()) {
		wlcm_e("setting %s failed, as the uAP is already running",
				argv[1]);
		return;
	}
	...
	if (!strcmp(argv[1], "sys_cfg_channel")) {
		...
		wmprintf("Channel setting successful\r\n");
		return;
	}
	if (!strcmp(argv[1], "sys_cfg_max_sta_num")) {
