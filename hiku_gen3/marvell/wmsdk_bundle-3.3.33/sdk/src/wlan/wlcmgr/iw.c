...
extern int wlan_memrdwr_getset(int argc, char *argv[]);
extern int cmd_wlan_getset_tx_power(int argc, char *argv[]);
extern int cmd_wlan_getset_tx_rate_index(int argc, char *argv[]);
...
static void dump_iwpriv_usage()
{
	...
	wmprintf("\r\niwpriv mlan0 memrdwr\r\n");
	wmprintf("This command is used to read/write the adapter memory\r\n");
	wmprintf("Usage:\r\n");
	wmprintf("iwpriv mlan0 memrdwr <address> [value]\r\n");
	wmprintf("Examples:\r\n");
	wmprintf("iwpriv mlan0 memrdwr 0x4cf70 - "
		"Read memory address 0x4cf70\r\n");
	wmprintf("iwpriv mlan0 memrdwr 0x80000000 0xffffffff - "
	...
	wmprintf("\r\nTo set the data rate index for"
		"Tx packet:\r\n");
	wmprintf("iwpriv <interface> tx_rate_index <index>\r\n");
	wmprintf("Data rate index:\r\n");
	wmprintf("   0 for 1M; 1 for 2M; 2 for 5.5M; 3 for 11M;"
		" 5 for 6M; 6 for 9M;\r\n");
	wmprintf("   7 for 12M; 8 for 18M; 9 for 24M; 10 for 36M;"
		" 11 for 48M; \r\n");
	wmprintf("   12 for MCS0; 13 for MCS1; 14 for MCS2; 15 for MCS3;"
		" 16 for MCS4; 17 for MCS5; 18 for MCS6; 19 for MCS7\r\n");
	wmprintf("   20 for MCS8; 21 for MCS9; 22 for MCS10; 23 for MCS11;"
		" 24 for MCS12; 25 for MCS13; 26 for MCS14; 27 for MCS15\r\n");
...
static void test_iwpriv_commands(int argc, char **argv)
{
	...
	if (argc == 3) {
		...
		if (!strcmp(argv[2], "tx_rate_index")) {
			cmd_wlan_getset_tx_rate_index(argc, argv);
			return;
		}
		...
		if (!strcmp(argv[2], "memrdwr")) {
			wlan_memrdwr_getset(argc - 1, argv + 1);
			return;
		}
