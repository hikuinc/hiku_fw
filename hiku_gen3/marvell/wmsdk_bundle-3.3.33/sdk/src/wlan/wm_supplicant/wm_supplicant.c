...
static const char *print_cmd_id(supplicant_cmd_id_t cmd_id)
{
	...
	case SUPP_STA_SET_PASSPHRASE:
		...
		break;
	case SUPP_STA_REMOVE_PASSPHRASE:
		ptr = "SUPP_STA_REMOVE_PASSPHRASE";
		break;
	case SUPP_STA_SET_PMK:
		ptr = "SUPP_STA_SET_PMK";
		break;
	case SUPP_STA_SESS_INIT:
		ptr = "SUPP_SESS_INIT";
		break;
	case SUPP_STA_SESS_FAILURE:
		ptr = "SUPP_STA_SESS_FAILURE";
		break;
		...
		bool is_pmf_required)
{
	...
	if (security > 0) {
		...
		memcpy(&msg->data.sinfo.ucstCipher, ucstCipher,
				sizeof(struct wlan_cipher));
		msg->data.sinfo.is_pmf_required = is_pmf_required;
	}
	...
	if (os_queue_send(&supplicant_queue, &msg, OS_NO_WAIT)
			!= WM_SUCCESS) {
		supp_e("Command %d: Failed to send cmd", cmd_id);
		os_mem_free(msg);
		return -WM_FAIL;
	}
	...
	while (1) {
		...
		case SUPP_STA_ENABLE:
			supplicantEnable(&sta_conn_info,
				...
				q_msg->data.sinfo.is_pmf_required);
			break;
		case SUPP_STA_SET_PASSPHRASE:
			set_psk(q_msg->data.ainfo.ssid,
		...
		case SUPP_STA_SESS_FAILURE:
			...
			break;
		case SUPP_STA_REMOVE_PASSPHRASE:
			remove_psk(q_msg->data.ainfo.ssid,
				strlen(q_msg->data.ainfo.ssid));
			break;
		case SUPP_STA_DEAUTH_DATA:
		...
		case SUPP_UAP_SET_PASSPHRASE:
			...
			if (q_msg->data.ainfo.passphrase)
