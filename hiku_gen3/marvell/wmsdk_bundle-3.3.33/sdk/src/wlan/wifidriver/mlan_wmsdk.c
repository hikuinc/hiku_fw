...
int wifi_set_rx_mgmt_indication(unsigned int mgmt_subtype_mask)
{
...
int wifi_send_remain_on_channel_cmd(wifi_remain_on_channel_t *remain_on_channel)
{
	wifi_get_command_lock();
	HostCmd_DS_COMMAND *cmd = wifi_get_command_buffer();
...
static char *get_country_str(int country)
{
	switch(country){
		case 1:
			return "US ";
...
static wifi_domain_param_t *get_11d_domain_params(int country, wifi_sub_band_set_t *sub_band, int nr_sb)
{
	...
	memcpy(dp->country_code, get_country_str(country), COUNTRY_CODE_LEN);
