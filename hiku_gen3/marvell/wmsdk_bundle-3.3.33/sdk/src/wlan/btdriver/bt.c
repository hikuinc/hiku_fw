...
static os_semaphore_t bt_bh_sem, bt_cmd_sem;
...
static void bt_interrupt(os_thread_arg_t data)
{
	...
	while (1) {
		...
		/* Unblock bt_drv_send() to send next command */
		if (sdio_ireg & DN_LD_HOST_INT_STATUS)
			os_semaphore_put(&bt_cmd_sem);
...
int32_t bt_drv_send(uint8_t pkt_type, uint8_t *data, uint32_t size)
{
	...
	/* Wait till the previous data is received  by the controller */
	os_semaphore_get(&bt_cmd_sem, OS_WAIT_FOREVER);
...
int32_t bt_drv_init(void)
{
	...
	if (os_semaphore_create(&bt_bh_sem, "bt-bh-sem") != WM_SUCCESS) {
		bt_log("Unable to create bt-bh semaphore");
		return -WM_FAIL;
	}
	os_semaphore_get(&bt_bh_sem, OS_NO_WAIT);


	if (os_semaphore_create(&bt_cmd_sem, "bt-cmd-sem") != WM_SUCCESS) {
		bt_log("Unable to create bt-cmd semaphore");
		return -WM_FAIL;
	}
