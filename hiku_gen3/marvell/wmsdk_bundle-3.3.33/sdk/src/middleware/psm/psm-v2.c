...
static int psm_part_scan(psm_t *psm)
{
	...
	while (offset < limit) {
		rv = psm_read_flash(psm, &pobj, sizeof(psm_object_t),
				    offset);
		if (rv != WM_SUCCESS) {
			psm_d("scan: could not read object @ off: %x", offset);
			break;
		}
		...
		if (pobj.object_type != OBJECT_TYPE_SMALL_BINARY) {
			psm_d("scan: Invalid obj at the end of data area: %x",
			      pobj.object_type);
			rv = -WM_FAIL;
			break;
		}
		...
		if (!psm_is_data_len_sane(psm, pobj.data_len)) {
			...
			psm_d("PSM invalid object found during scan.");
			rv = -WM_FAIL;
			break;
		}
		...
		rv = test_and_set_bitmap(bitmap, pobj.obj_id);
		if (rv == -WM_FAIL) {
			psm_d("Double active object %d detected", pobj.obj_id);
...
/*
 * - Copies and object from source offset to dest offset.
			   ...
			   objectid_t new_objid)
{
	...
	if (rv != WM_SUCCESS) {
		psm_d("Failed to marke copied object inactive");
		return rv;
	}
				...
				uint32_t dest_offset, uint32_t dest_limit)
{
	...
	while (source_offset < source_limit) {
		...
		rv = psm_copy_object(psm, &pobj, source_offset, dest_offset,
				     ++objid_counter);
		if (rv != WM_SUCCESS) {
			psm_d("compaction failed");
			return rv;
		}
			   ...
			   psm_swap_desc_t *swap_desc)
{
	...
	case CHECKPOINT_1:{
		...
		rv = psm_copy_all_objects(psm, data_offset, data_limit,
					  swap_data_offset, swap_limit);
		if (rv != WM_SUCCESS) {
			psm_d("CHK-1: copy to swap failed");
			return rv;
		}
		if (g_psm_event_cb)
			g_psm_event_cb(PSM_EVENT_COMPACTION,
	...
	case CHECKPOINT_2:{
		...
		rv = psm_copy_all_objects(psm, swap_data_offset,
					  swap_limit, data_offset,
					  data_limit);
		if (rv != WM_SUCCESS) {
				 ...
				 uint32_t offset, uint32_t limit)
{
	...
	if (pobj->flags & PSM_OBJ_INACTIVE) {
...
static int psm_module_physical_init(psm_t *psm)
{
	if (psm->psm_cfg.read_only) {
		/*
		 * If read-only is enabled, no recovery procedure can be
		 * done even if necessary. Only a scan is done. Note that
		 * since no recovery can be done, there is more probability
		 * of scan to fail.
		 */
		return psm_part_scan(psm);
	}
	...
	if (checkpoint != CHECKPOINT_2) {
		...
		if (rv != WM_SUCCESS) {
			 ...
			 * compaction may salvage intact objects.
			 */
...
static psm_t *_get_existing_psm_object(const flash_desc_t *fdesc)
{
	...
	psm_t *psm;
	list_for_each_entry(psm, &part_list, node) {
		    ...
		    psm_cfg_t *psm_cfg)
{
	...
	} else { /* Security functionality is present but will be unused */
		psm_d("Assuming non secure partition");
	}
#endif /* CONFIG_SECURE_PSM */
