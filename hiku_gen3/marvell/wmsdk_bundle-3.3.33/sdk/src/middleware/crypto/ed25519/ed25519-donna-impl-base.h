...
#ifdef ED25519_FLASH_DATA
#include <partition.h>
#include <flash.h>
#include <mdev.h>

#define ED25519_PART_NAME	"ed_data"

static flash_desc_t ed25519_fl_desc;
static uint8_t valid_fl_desc;

static int read_ed25519_flash_descriptor(void)
{
	...
	if (part_init()) {
		wmprintf("part init failed\r\n");
		return -1;
	}

	p = part_get_layout_by_name(ED25519_PART_NAME, NULL);
	if (!p) {
		return -2;
	}

	part_to_flash_desc(p, &ed25519_fl_desc);
	valid_fl_desc = true;

	return 0;
}

int read_ed25519_rodata(uint32_t pos, uint8_t *buf, uint32_t len)
{
	int ret = 0;
	mdev_t *fl_dev;

	if (!valid_fl_desc) {
		ret = read_ed25519_flash_descriptor();
		if (ret)
			return ret;
	}

	fl_dev = flash_drv_open(ed25519_fl_desc.fl_dev);
	if (!fl_dev)
		return -3;

	/* flash read out of bound */
	if (ed25519_fl_desc.fl_size < (pos + len))
		return -4;

	ret = flash_drv_read(fl_dev, buf, len, ed25519_fl_desc.fl_start + pos);
...
ge25519_scalarmult_base_choose_niels(ge25519_niels *t, const uint8_t table[256][96], uint32_t pos, signed char b) {
	...
	uint32_t i;
#ifdef ED25519_FLASH_DATA
	static uint8_t partial_rodata[8][96];
#endif
...
#ifdef ED25519_FLASH_DATA
	read_ed25519_rodata(pos * 8 * 96, (uint8_t *) partial_rodata, sizeof(partial_rodata));
#endif

	for (i = 0; i < 8; i++)
#ifdef ED25519_FLASH_DATA
		curve25519_move_conditional_bytes(packed, partial_rodata[i], ge25519_windowb_equal(u, i + 1));
#else
