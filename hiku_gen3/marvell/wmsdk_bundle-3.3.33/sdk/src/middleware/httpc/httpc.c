...
int httpc_write_chunked(http_session_t handle, const char *data, int len)
{
	...
	if (!(s->state == SESSION_REQ_DONE &&
		...
		httpc_d("Cannot write data. session state: %d,"
...
static int prefetch_header(session_t *s)
{
	...
	httpc_d("Header not found.");
	httpc_d("The stream may be in inconsistent state or");
	httpc_d("in a rare case, the header may be bigger that %d bytes",
			...
			unsigned int socket_timeout)
{
	...
	if (*sockfd < 0) {
		httpc_d("Unable to create socket");
	...
	if (setsockopt(*sockfd, SOL_SOCKET, SO_SNDBUF,
		...
		httpc_d("Unsupported option SO_SNDBUF: %d",
	...
	if (setsockopt(*sockfd, SOL_SOCKET, SO_RCVBUF,
		       (char *)&opt, sizeof(opt)) == -1) {
		httpc_d("Unsupported option SO_RCVBUF: %d",
			net_get_sock_error(*sockfd));
	}
...
#ifdef HTTPC_TCP_NODELAY
	{
		...
		if (res < 0) {
			httpc_d("setsockopt TCP_NODELAY failed");
			net_close(*sockfd);
			*sockfd = 0;
			return -WM_FAIL;
		}
...

{
	...
	if (r != WM_SUCCESS) {
		httpc_d("Socket creation for %s:%d failed", hostname, port);
		return r;
	}
...
static inline session_t *new_session_object()
{
	...
	if (!s) {
		httpc_d("Could not allocate session object");
		return NULL;
	}
		...
		if (token) {
			*token = 0;
			...
			if (sscanf(token, "%u", &parsed_url->portno) != 1) {
				httpc_d("Port number parsing failed: %s",
					token);
				return -WM_FAIL;
			}
		...
		if (parsed_url->hostname[0] != '[')
			...
			if (token) {
				...
				if (ptr) {
					...
					if (sscanf(ptr, "%u",
						&parsed_url->portno) != 1) {
						httpc_d("Port number parsing "
						"failed for IPv6: %s", ptr);
						return -WM_FAIL;
					} else {
						/* Terminate hostname at ']' */
						*token = 0;
						httpc_d("Parse: IPv6 Hostname: "
		   ...
		   parsed_url_t *parsed_url)
{
	...
	if (min_size_req > tmp_buf_len) {
		httpc_d("Cannot parse URL");
		httpc_d("Temporary buffer size is less than required");
		return -WM_E_INVAL;
	}
		      ...
		      const httpc_cfg_t *httpc_cfg)
{
	...
	if (s->httpc_cfg.flags & TLS_ENABLE) {
		httpc_d("ENABLE_TLS and/or ENABLE_HTTPC_SECURE are " \
			"not enabled in configuration.");
		httpc_d("Cannot set up a TLS connection.");
		delete_session_object(s);
...
static const char *get_method_string(http_method_t type)
{
	...
	case HTTP_CONNECT:	/* do we need this  ? */
		httpc_d("Method not yet supported.");
		return NULL;
	default:
		httpc_d("Unknown method given.");
	}
...
static int _http_retrieve_and_send_content(session_t *s, const http_req_t *req)
{
	...
	while (remaining) {
		...
		to_send = req->content_fetch_cb(http_session, buf,
						to_send);
		if (to_send <= 0) {
			httpc_d("Content recv cb failed");
			return to_send;
		}
			     ...
			     http_hdr_field_sel_t field_flags)
{
	...
	if (!resource) {
		httpc_d("HTTPC: resource invalid");
		s->state = SESSION_ERROR;
		return -WM_E_INVAL;
	}
		     ...
		     http_hdr_field_sel_t field_flags)
{
	...
	if (!handle || !req) {
		httpc_d("Cannot prepare request, invalid parameters."
			" State: %d", s->state);
		return -WM_E_INVAL;
	}
	if (s->state != SESSION_INIT_DONE &&
	    s->state != SESSION_REQ_DONE)  {
		httpc_d("Cannot prepare request, invalid session state."
			" State: %d", s->state);
		return -WM_E_INVAL;
	}
	/*
...
int httpc_validate_url(const char *url_str)
{
	...
	if (!tmp_buf) {
		httpc_d("Mem allocation failed. Tried size: %d",
	      ...
	      http_resp_t **http_resp, const httpc_cfg_t *httpc_cfg)
{
	...
	if (status != WM_SUCCESS) {
		httpc_d("Open session failed URL: %s", url_str);
	...
	if (status != WM_SUCCESS) {
		httpc_d("preparing request failed: %s", url_str);
	...
	if (status != WM_SUCCESS) {
		httpc_d("Request send failed: URL: %s", url_str);
		goto http_session_error;
	}

	status = http_get_response_hdr(*handle, http_resp);
	if (status != WM_SUCCESS) {
		httpc_d("Failed to get response header: URL: %s", url_str);
		goto http_session_error;
	}
	return WM_SUCCESS;
http_session_error:
...
int http_get_response_hdr(http_session_t handle, http_resp_t **resp)
{
	...
	session_t *s = (session_t *) handle;
	if (!handle) {
		httpc_d("Unable to get resp header, invalid params."
			" State: %d", s->state);
		return -WM_E_INVAL;
	}
	if (s->state != SESSION_REQ_DONE &&
		...
		httpc_d("Unable to get resp header, invalid session state."
		" State: %d", s->state);
		return -WM_E_INVAL;
	}
	...
	if (r == WM_SUCCESS) {
		...
		if (!s->resp.content_length_field_present && !s->resp.chunked) {
			 ...
			 * connection */
			httpc_d("No content length known. Read till server "
			      ...
			      int *count)
{
	...
	case SESSION_INIT_DONE:
		if (s->outstanding_requests > 0)
			break;
	default:
		httpc_d("Invalid state: %d. Cannot read hdr value", s->state);
		return -WM_E_INVAL;
	}
...
static int get_content_len(session_t *s)
{
	...
	for (i = 0; i < MAX_CHUNK_HEADER_SIZE; i++) {
		...
		if (size_read < 0) {
			httpc_d("Error while reading chunk len: %d",
				size_read);
			return size_read;
		}
		...
		if (first_delim_found) {
			if (chunk_header[i] != '\n') {
				httpc_d("Unable to read len. No newline");
				return -WM_FAIL;
			}
			break;
		}
	...
	if (s->chunk.state == STATE_BETWEEN_CHUNK) {
		...
		if (s->chunk.size == 0) {
			...
			if (zap_chunk_boundary(s) == WM_SUCCESS) {
				...
				httpc_d("At end of transaction, ");
				httpc_d("could not zap chunk boundary");
				return -WM_FAIL;
			}
		}
		int to_read =
...
int http_read_content(http_session_t handle, void *buf, uint32_t max_len)
{
	...
	case SESSION_INIT_DONE:
		if (s->outstanding_requests == 0) {
			httpc_d("Cannot read content, no outstanding requests"
