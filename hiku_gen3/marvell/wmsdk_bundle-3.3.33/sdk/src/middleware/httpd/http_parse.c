 ...
 * httpd_request_t structure. Note that this function will clobber the
 * given data buffer containing the header line */
				  ...
				  httpd_request_t *req_p, uint8_t *done)
{
	...
	const char *name = data_p;
	char *ptr = index(data_p, ':');
	if (!ptr) {
		/* colon was not found. The line is invalid */
		httpd_d("No colon found. Invalid buffer");
		return -WM_FAIL;
	}

	/* NULL terminate the 'name' */
	*ptr = 0;
	const char *value = ptr + 2;

	if (!strcasecmp(name, http_content_len)) {
		req_p->body_nbytes = atol(value);
		if (req_p->body_nbytes == 0) {
			httpd_d("got 0 for body length");
			return WM_SUCCESS;
		}
		req_p->remaining_bytes = req_p->body_nbytes;
	} else if (!strcasecmp(name, http_user_agent)) {
		httpd_parse_useragent((char *)value, &req_p->agent);
	} else if (!strcasecmp(name, http_content_type)) {
		strncpy(req_p->content_type, value,
			HTTPD_MAX_CONTENT_TYPE_LENGTH);
	} else if (!strcasecmp(name, http_if_none_match)) {
		/*
		 * We use the FTFS CRC to generate ETag. Hence, the ETag we
		 * receive is not expected to be more that 32 bits in value.
		 */
		const char *first_double_quote = strchr(value, '"');
	...
	} else if (!strcasecmp(name, http_encoding)) {
	...
	char *buffer, int len, httpd_parse_hdr_cb_t cb)
{
	...
	while (TRUE) {
		...
		if (cb && (req_line_len != 0)) {
			(*cb)(buffer, req_line_len, req);
		}
...
{
	return httpd_parse_hdr_tags_with_cb(req, sock, buffer, len, NULL);
}
...
static int httpd_verify_req_type(const char *token_p, int *type)
{
	...
	if (!strncmp(token_p, http_post, strlen(http_post)))
		*type = HTTPD_REQ_TYPE_POST;
	else if (!strncmp(token_p, http_get, strlen(http_get)))
		*type = HTTPD_REQ_TYPE_GET;
	else if (!strncmp(token_p, http_put, strlen(http_put)))
		*type = HTTPD_REQ_TYPE_PUT;
	else if (!strncmp(token_p, http_delete, strlen(http_delete)))
		*type = HTTPD_REQ_TYPE_DELETE;
	else if (!strncmp(token_p, http_head, strlen(http_head)))
