				   ...
				   bool send_cache_headers, unsigned etag_val)
{
	...
	err = httpd_send_str(conn, statushdr);
	if (err != WM_SUCCESS) {
		return err;
	}

	/* Set encoding to gz if so */
	if (!strncmp(encoding, http_gz, strlen(http_gz))) {
		err = httpd_send_str(conn, http_content_encoding_gz);
		if (err != WM_SUCCESS)
			return err;
	}

	if (send_cache_headers &&
	    !strncmp(http_shtml, file_type, strlen(http_shtml))) {
		err = httpd_send_str(conn, http_cache_control);
		if (err != WM_SUCCESS)
			return err;
	...
	if (file_type == NULL) {
		err = httpd_send_str(conn, http_content_type_binary);
	} else if (!strncmp(http_manifest, file_type, strlen(http_manifest))) {
		err = httpd_send_str(conn,
				     http_content_type_text_cache_manifest);
	} else if (!strncmp(http_html, file_type, strlen(http_html))) {
		err = httpd_send_str(conn, http_content_type_html);
	} else if (!strncmp(http_xml, file_type, strlen(http_xml))) {
		err = httpd_send_str(conn, http_content_type_xml);
	} else if (!strncmp(http_shtml, file_type, strlen(http_shtml))) {
		err = httpd_send_str(conn, http_content_type_html_nocache);
	} else if (!strncmp(http_css, file_type, strlen(http_css))) {
		err = httpd_send_str(conn, http_content_type_css);
	} else if (!strncmp(http_js, file_type, strlen(http_js))) {
		err = httpd_send_str(conn, http_content_type_js);
	} else if (!strncmp(http_png, file_type, strlen(http_png))) {
		err = httpd_send_str(conn, http_content_type_png);
	} else if (!strncmp(http_gif, file_type, strlen(http_gif))) {
		err = httpd_send_str(conn, http_content_type_gif);
	} else if (!strncmp(http_jpg, file_type, strlen(http_jpg))) {
		err = httpd_send_str(conn, http_content_type_jpg);
	} else {
		err = httpd_send_str(conn, http_content_type_plain);
	}
