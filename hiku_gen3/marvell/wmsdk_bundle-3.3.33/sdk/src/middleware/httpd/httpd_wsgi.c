...
int httpd_register_wsgi_handler(struct httpd_wsgi_call *wsgi_call)
{
	...
	for (i = 0; i < MAX_WSGI_HANDLERS; i++) {
		...
		if (calls[i] && (strcmp(calls[i]->uri, wsgi_call->uri) == 0)) {
...
int httpd_check_if_last_line(int sock)
{
	unsigned char ch;
	int ret;
	if ((ret = httpd_recv(sock, &ch, 1, 0)) <= 0)
		return -WM_FAIL;
	if (ch != '\r')
		return -WM_FAIL;
	if ((ret = httpd_recv(sock, &ch, 1, 0)) <= 0)
		...
		*content_type, char *content, int content_len)
{
	...
	if (req->type == HTTPD_REQ_TYPE_GET ||
		...
		if (ret != WM_SUCCESS) {
			httpd_e("Unable to purge headers: %p",
				...
				int length, const char *content_type)
{
	...
	if (!req->hdr_parsed && (req->type == HTTPD_REQ_TYPE_GET ||
		req->type == HTTPD_REQ_TYPE_HEAD)) {
		ret = httpd_check_if_last_line(req->sock);
		if (ret != WM_SUCCESS) {
			ret = httpd_purge_headers(req->sock);
	...
	if (content_type) {
		ret = httpd_send_header(req->sock, "Content-Type",
					content_type);
		if (ret != WM_SUCCESS) {
			httpd_e("Error in sending Content-Type");
			return ret;
			...
			char *content, int length, const char *content_type)
{
	...
	ret = httpd_send_response_header(req, first_line,
					length, content_type);
	if (ret != WM_SUCCESS)
		return ret;
...
int httpd_get_data(httpd_request_t *req, char *content, int length)
{
	...
	if (!req->hdr_parsed) {
		/* Use passed buffer itself for parsing header tags, iff given
		 * buffer is larger than HTTPD_MAX_MESSAGE. */
		if (length < HTTPD_MAX_MESSAGE) {
		...
		if (length < HTTPD_MAX_MESSAGE)
			...
			/* Clear the content buffer as it will have some
			 * data read during the header parsing above
			 */
			memset(content, 0, length);

		if (ret != WM_SUCCESS) {
			httpd_e("Unable to parse header tags: %p",
				__builtin_return_address(0));
			return ret;
		} else {
	...
	/* Handle here */
	/* Never demand for bytes more than total size of a body. Also
	 * if part of data for a particular request has already been sent out,
	 * successive calls to httpd_get_data should request only remaining
	 * bytes */
	length = (length >= req->remaining_bytes) ? req->remaining_bytes :
		length;
	/* If the size of data to be read comes out to be 0, just return 0
	 * as behavior of recv() can be unpredictable in this case.
	 */
	if (length == 0)
		return length;
	...
	ret = httpd_recv(req->sock, content,
			length, 0);
	if (ret <= 0) {
		httpd_e("Could not read POST data. Ret = %d", ret);
...
int httpd_wsgi(httpd_request_t *req_p)
{
	...
	httpd_assign_priv_data(req_p->sock, req_p->httpd_priv,
			req_p->free_priv_data_fn);
	if (err == WM_SUCCESS)
		return HTTPD_DONE;
	else
		return err;
