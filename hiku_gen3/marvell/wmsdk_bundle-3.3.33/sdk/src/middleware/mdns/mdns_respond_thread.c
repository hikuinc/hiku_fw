...
static int mc6_sock;
#else
/* This will make sure that if CONFIG_IPV6 is not enabled, mdns_send_msg won't
 * send IPv6 packets. */
...
static int get_config_idx_from_iface(void *iface)
{
	int i;
...
static int send_close_probe(int config_idx, struct mdns_service *services[])
{
	...
	return mdns_send_msg(&tx_msg, mc_sock, mc6_sock, htons(5353),
			config_g[config_idx].iface_handle, 0);
...
int mdns_verify_service(struct mdns_service *services[], void *iface)
{
	int config_idx = -1, i, num_services = 0, ret;
	struct mdns_service **tmp_services = services;

	if (iface == NULL || services == NULL)
		return -WM_E_MDNS_INVAL;
	...
	for (i = MDNS_MAX_SERVICE_CONFIG - 1; i >= 0; i--) {
		if (config_g[i].iface_handle == iface) {
			config_idx = i;
			break;
		} else if (config_g[i].iface_handle == NULL) {
			config_idx = i;
		}
	}
...
int mdns_add_service_iface(struct mdns_service *services[], void *iface)
{
	...
	config_g[config_idx].iface_handle = iface;
...
int mdns_remove_service_iface(struct mdns_service *services[], void *iface)
{
	int config_idx;
	...
	if (config_g[config_idx].num_services <= 0) {
		...
		config_g[config_idx].iface_handle = NULL;
	}

	return WM_SUCCESS;
}

int mdns_remove_all_services(void *iface)
{
	int i, config_idx;
...
#ifdef CONFIG_IPV6
		active_fds = net_select(mc6_sock + 1, &fds, NULL, NULL,
				timeout);
#else
		active_fds = net_select(mc_sock + 1, &fds, NULL, NULL, timeout);
#endif	/*	CONFIG_IPV6	*/
...
static inline void mdns_ctrl_iface_up(void *iface, int *state, int *event,
		struct timeval *probe_wait_time)
{
...
static inline void mdns_ctrl_iface_down(void *iface)
{
	int config_idx = get_config_idx_from_iface(iface);
	if (config_idx == -WM_FAIL) {
		LOG("No interface found.\r\n");
...
static inline void mdns_ctrl_reannounce(void *iface, int *state)
{
	int i, ret;
	int config_idx = get_config_idx_from_iface(iface);
	if (config_idx == -WM_FAIL) {
...
static inline void mdns_ctrl_announce(void *service, void *iface, int *state,
...
static inline void mdns_ctrl_deannounce(void *service, void *iface)
{
	int config_idx;
	struct mdns_service *service_arr[2];
...
static inline void mdns_ctrl_announce_arr(void *services, void *iface,
...
static inline void mdns_ctrl_deannounce_arr(void *service, void *iface)
{
	int config_idx = get_config_idx_from_iface(iface);
	if (config_idx == -WM_FAIL) {
...
static inline void mdns_ctrl_deannounce_all(void *iface)
{
	int config_idx = get_config_idx_from_iface(iface);
	if (interface_state[config_idx] != STOPPED) {
		send_close_probe(config_idx, NULL);
...
static void do_responder(void)
{
	...
	ret = os_mutex_create(&mdns_mutex, "mdns", OS_MUTEX_INHERIT);
	if (ret != WM_SUCCESS)
		return;
	for (i = 0; i < MDNS_MAX_SERVICE_CONFIG; i++) {
	...
	while (responder_enabled) {
		...
		if (FD_ISSET(ctrl_sock, &fds)) {
			...
			in_size = sizeof(struct sockaddr_in);
			ret =
			    recvfrom(ctrl_sock, (char *)&msg, sizeof(
				     mdns_ctrl_data),
				     MSG_DONTWAIT, (struct sockaddr *)&from,
				     &in_size);
			if (ret == -1) {
				LOG("Warning: responder failed to get control"
						" message\r\n");
			} else {
...
int responder_halt(void)
{
	...
	for (i = 0; i < MDNS_MAX_SERVICE_CONFIG; i++) {
		mdns_remove_all_services(config_g[i].iface_handle);
		config_g[i].iface_handle = NULL;
		interface_state[i] = STOPPED;
	}
