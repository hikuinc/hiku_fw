...
static void display_ping_result(ip_addr_t *addr, int total, int recvd)
{
	...
	wmprintf("\r\n--- %s ping statistics ---\r\n", inet_ntoa(*addr));
	wmprintf("%d packets transmitted, %d received,", total, recvd);
	if (dropped != 0)
		wmprintf(" +%d errors,", dropped);
	wmprintf(" %d%% packet loss\r\n", (dropped * 100) / total);
}

/* Display the statistics of the current iteration of ping */
static void display_ping_stats(int status, uint32_t size, ip_addr_t *ipaddr,
		int seqno, int ttl, uint32_t time)
{
	if (status == WM_SUCCESS)
		wmprintf("%u bytes from %s: icmp_req=%u ttl=%u time=%u ms\r\n",
			size, inet_ntoa(*ipaddr), seqno, ttl, time);
	else
		wmprintf("From %s icmp_seq=%u Destination Host Unreachable\r\n",
...
{
	wmprintf("Usage:\r\n");
	wmprintf("\tping [-s <packet_size>] [-c <packet_count>] "
		"[-W <timeout in sec>] <ip_address>\r\n");
	wmprintf("Default values:\r\n");
	wmprintf("\tpacket_size: %u\r\n\tpacket_count: %u"
		"\r\n\ttimeout: %u sec\r\n", PING_DEFAULT_SIZE,
		PING_DEFAULT_COUNT, PING_DEFAULT_TIMEOUT_SEC);
}
 ...
 * result */
static int ping(unsigned int count, unsigned short size,
		unsigned int timeout, ip_addr_t *addr)
{
	...
	wmprintf("PING %s (%s) %u(%u) bytes of data\r\n", inet_ntoa(*addr),
			inet_ntoa(*addr), size, size + sizeof(struct ip_hdr) +
			sizeof(struct icmp_echo_hdr));

	/* Create a raw socket */
	s = socket(AF_INET, SOCK_RAW, 1);
	if (s < 0) {
		nw_utils_e("Failed to create raw socket for ping %d", s);
		return -WM_FAIL;
	}
	...
	ret = setsockopt(s, SOL_SOCKET, SO_RCVTIMEO,
				&timeout, sizeof(int));
	if (ret < 0) {
		nw_utils_e("Failed to set socket options for ping");
		ret = -WM_FAIL;
		goto end;
	}
	...
	if (!iecho) {
		nw_utils_e("Failed to allocate memory for ping packet");
		ret = -WM_FAIL;
		goto end;
	}
	...
	while (i <= count) {
		...
		ret = lwip_sendto(s, iecho, ping_size, 0,
		...
		if (ret > 0) {
			...
			display_ping_stats(ret, ping_size, ip_addr, i, ttl,
				ping_time);
		} else {
			nw_utils_e("Failed to send ping echo request");
			os_mem_free(iecho);
			ret = -WM_FAIL;
			goto end;
		}
	...
	while ((c = cli_getopt(argc, argv, "c:s:W:")) != -1) {
		...
		case 's':
			...
			if (temp > PING_MAX_SIZE) {
				wmprintf("ping: packet size too large: %u."
					" Maximum is %u\r\n", temp,
					PING_MAX_SIZE);
				return;
			}
...
static struct cli_command nw_utils[] = {
	{"ping", NULL, cmd_ping},
};

int nw_utils_cli_init(void)
{
	int i;
	for (i = 0; i < sizeof(nw_utils) / sizeof(struct cli_command); i++)
		if (cli_register_command(&nw_utils[i]))
			return -WM_FAIL;
	return WM_SUCCESS;
}

int nw_utils_cli_deinit(void)
{
	int i;

	for (i = 0; i < sizeof(nw_utils) / sizeof(struct cli_command); i++)
		if (cli_unregister_command(&nw_utils[i]))
			return -WM_FAIL;
	return WM_SUCCESS;
