...
int app_sys_http_update_all(jobj_t *jobj, short *fs_done,
			    short *fw_done, short *wififw_done)
{
	 ...
	 */
	if (json_get_val_str(jobj, J_NAME_FSURL, update_url,
	...
	if (json_get_val_str(jobj, J_NAME_FWURL, update_url,
			     sizeof(update_url)) == WM_SUCCESS) {
...
#ifdef CONFIG_WIFI_FW_UPGRADE
	if (json_get_val_str(jobj, J_NAME_WIFIFWURL, update_url,
			     sizeof(update_url)) == WM_SUCCESS) {
...
#define MEGA (1 << 10 << 10)
	if (perf > (MEGA * 8)) {
		...
		perf = perf / MEGA;
		snprintf(perf_resp, sizeof(perf_resp), "time = %lums "
...
int sys_webupdate(httpd_request_t *req)
{
	...
	if (!httpd_validate_uri(req->filename,
		...
		if (ret != WM_SUCCESS)
			return -WM_FAIL;
		err = app_sys_http_update_all(&jobj, NULL, NULL, NULL);
		goto end;
	}
end:
	if (mp_header) {
...
int sys_handle_webcmd(jobj_t *jobj)
{
	int ret = -1;
	char *tag = json_val;

	if (json_get_val_str(jobj, J_NAME_COMMAND, tag, MAX_JSON_VAL)
	    != WM_SUCCESS) {
...
int sys_webcmd(httpd_request_t *req)
{
	...
	ret = httpd_get_data(req, sys_cmd_req, sizeof(sys_cmd_req));
	if (ret < 0) {
		app_e("Failed to get post request data");
		return ret;
	}
	...
	jobj_t jobj;
	ret = json_init(&jobj, json_tokens, NUM_TOKENS,
			sys_cmd_req, req->body_nbytes);
	if (ret != WM_SUCCESS)
		return -WM_FAIL;

	ret = sys_handle_webcmd(&jobj);
	if (ret == WM_SUCCESS)
...
int sys_validate_name(char *name)
{
	...
	for (i = 0; i < len; i++) {
		if ((name[i] != '-') && (!isalnum((unsigned char)name[i])))
			return 1;
...
int sys_set_info(httpd_request_t *req)
{
	...
	ret = httpd_get_data(req, sys_info_req,
			sizeof(sys_info_req));
	/* check condition */
	if (ret < 0) {
		app_e("Failed to get post request data");
	...
	jobj_t jobj;
	ret = json_init(&jobj, json_tokens, NUM_TOKENS,
			sys_info_req, req->body_nbytes);
	if (ret != WM_SUCCESS)
		return -WM_FAIL;

	if (json_get_composite_object(&jobj,
			    J_NAME_PROVISIONING) == WM_SUCCESS)	{
		if (json_get_val_int(&jobj, J_NAME_CLIENT_ACK,
				     &ack) == WM_SUCCESS) {
			handle_client_ack = 1;
		}
		json_release_composite_object(&jobj);
	}
	if (json_get_composite_object(&jobj, J_NAME_CONNECTION) == WM_SUCCESS) {
		if (json_get_composite_object(&jobj, J_NAME_STA)
				== WM_SUCCESS) {
			if (json_get_val_int(&jobj, "configured",
					     &reset) == WM_SUCCESS) {
				handle_configure = 1;
			}
			json_release_composite_object(&jobj);
...
int sys_handle_set_time(jobj_t *jobj)
{
	time_t time;
	int ret = -1;
...
int sys_set_time(httpd_request_t *req)
{
	...
	jobj_t jobj;
	ret = json_init(&jobj, json_tokens, NUM_TOKENS,
			sys_time_req, req->body_nbytes);
	if (ret != WM_SUCCESS)
		return -WM_FAIL;

	ret = sys_handle_set_time(&jobj);
	if (ret == WM_SUCCESS)
		ret = httpd_send_response(req, HTTP_RES_200, HTTPD_JSON_SUCCESS,
			strlen(HTTPD_JSON_SUCCESS), HTTP_CONTENT_JSON_STR);
	else
...
int sys_set_mode(httpd_request_t *req)
{
	...
	ret = httpd_get_data(req, sys_mode_req,
			sizeof(sys_mode_req));
	if (ret < 0) {
		app_e("Failed to get post request data");
		return ret;
	...
	jobj_t jobj;
	ret = json_init(&jobj, json_tokens, NUM_TOKENS,
			sys_mode_req, req->body_nbytes);
	if (ret != WM_SUCCESS)
		return -WM_FAIL;

	err = json_get_val_int(&jobj, J_NAME_MODE, &mode);
	if (err != WM_SUCCESS) {
		ret = httpd_send_response(req, HTTP_RES_200, HTTPD_JSON_ERROR,
			strlen(HTTPD_JSON_ERROR), HTTP_CONTENT_JSON_STR);
	} else {
...
#define OTHER       "other"
	...
	if (is_sta_connected()) {
		...
		if (is_sta_ipv4_connected()) {
			...
			json_set_val_int(&jstr,
				J_NAME_IPTYPE, network.ip.ipv4.addr_type);
		...
		if (is_sta_ipv6_connected()) {
			...
			for (i = 0; i < MAX_IPV6_ADDRESSES; i++) {
				if (network.ip.ipv6[i].addr_state !=
					IP6_ADDR_INVALID) {
 ...
 sta_err:
	...
	if (is_uap_started()) {
		...
		json_set_val_int
			(&jstr, J_NAME_IPTYPE, network.ip.ipv4.addr_type);
...
int sys_get_diag_info(httpd_request_t *req)
{
	...
	if (mode) {
		...
		if (ret == WM_SUCCESS && state == WLAN_CONNECTED) {
		...
		if (is_sta_ipv4_connected()) {
			json_push_object(&jstr, J_NAME_IPV4);
			json_set_val_str(&jstr, J_NAME_IP_ADDR,
				 inet_ntoa(network.ip.ipv4.address));
			json_pop_object(&jstr); /* J_NAME_IPV4 */
		}
#ifdef CONFIG_IPV6
		if (is_sta_ipv6_connected()) {
...
int app_sys_register_file_handler()
{
	...
	rc = httpd_register_wsgi_handlers(&app_file_handler_wsgi, 1);
	if (rc) {
...
int app_sys_normal_set_network_cb()
{
	...
	rc = set_network_cb_register(app_sta_save_network_and_start);
	if (rc) {
...
static inline int sys_set_prov_status(httpd_request_t *req)
{
	...
	ret = httpd_get_data(req, sys_prov_st_req,
			sizeof(sys_prov_st_req));
	/* check condition */
	if (ret < 0) {
		app_e("Failed to get post request data");
		return ret;
	...
	jobj_t jobj;
	ret = json_init(&jobj, json_tokens, NUM_TOKENS,
			sys_prov_st_req, req->body_nbytes);
	if (ret != WM_SUCCESS)
		return -WM_FAIL;

	if (json_get_val_int(&jobj, J_NAME_PROV_FINISH, &status)
			!= WM_SUCCESS)
		return -WM_FAIL;
