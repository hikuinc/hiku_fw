...
static os_thread_stack_define(app_ctrl_event_stack, 2560);

/* app ctrl event processing thread */
static os_thread_t app_ctrl_event_thread;
extern int app_get_current_uap_network();
...
const char *app_ctrl_event_name(app_ctrl_event_t event)
{
	...
	case AF_EVT_UAP_CLIENT_ASSOC:
		ptr = "UAP_CLIENT_ASSOC";
		break;
	case AF_EVT_UAP_CLIENT_DISSOC:
		ptr = "UAP_CLIENT_DISSOC";
		break;
...
static int app_ctrl_state_init_system(struct app_init_state *state)
{
	...
	app_l("app_ctrl: prev_fw_version=%d", boot_old_fw_version());
...
static void app_ctrl_prov_network_set(struct wlan_network *net)
{
	...
	if (ret != WM_SUCCESS) {
		 ...
		 */
		psm_format(app_psm_hnd);
		app_reboot(REASON_NW_ADD_FAIL);
	}
}

static void app_ctrl_handle_event_uap_started()
{
...
static void app_ctrl_p2p_sm(app_ctrl_event_t event, app_ctrl_data_t *data)
{
	...
	case AF_STATE_P2P_INIT:
		if (event == AF_EVT_INTERNAL_P2P_REQUESTED) {
			...
			if (((data->app_data).p2p_role_data).role
			    == ROLE_AUTO_GO)
	...
	case AF_STATE_P2P_SESSION_STARTED:
		if (event == AF_EVT_P2P_ROLE_NEGOTIATED) {
			if (((data->app_data).p2p_role_data).role == ROLE_GO) {
	...
	case AF_STATE_P2P_SESSION_COMPLETE:
		if (event == AF_EVT_INTERNAL_STATE_ENTER) {
			app_connect_to_p2p_go(&(data->app_data).current_net);
...
#endif /* CONFIG_P2P */
static void app_ctrl_uap_sm(app_ctrl_event_t event, app_ctrl_data_t *data)
{
	int ret;
	...
	case AF_STATE_UAP_INIT:
		if (event == AF_EVT_INTERNAL_UAP_REQUESTED) {
			...
			if (ret != WM_SUCCESS)
				critical_error(-CRIT_ERR_APP_FRAMEWORK, NULL);
			s.uap_next = AF_STATE_UAP_STARTING;
			event_handled = 1;
		} else if (event == AF_EVT_UAP_STARTED) {
			/* Handled this event in UAP_INIT state for EZConnect
			 * provisioning. In smc mode, when device receives uAP
			 * up request, smc mode is stopped and uAP is started
			 * using API wlan_stop_smart_mode_start_uap. As uap is
			 * started in the context of wlcmgr, uap_starting event
			 * is not generated and wlcmgr notifies af when uap is
			 * started */
			if (app_get_current_uap_network() != WM_SUCCESS) {
				app_d("Failed to configure uap network");
			}
			app_ctrl_handle_event_uap_started();
			event_handled = 1;
		}
...
static int app_ctrl_prov_sm(app_ctrl_event_t event,  app_ctrl_data_t *data)
{
...
#endif
		/* AF_EVT_NW_SET event can be received
		 * here from uAP provisioning.
		 */
		if (event == AF_EVT_NW_SET &&
		    data->ptype == APP_DEFAULT_PROVISIONING) {
			app_ctrl_event_to_prj(AF_EVT_PROV_DONE,
					      &((data->app_data).current_net));
...
void app_set_state_configured()
{
	if (s.st_curr == AF_STATE_UNCONFIGURED) {
		s.st_curr = AF_STATE_CONFIGURED;
		s.st_next = AF_STATE_CONFIGURED;
	}
}

static void app_ctrl_sm(app_ctrl_event_t event, app_ctrl_data_t *data)
{
	int ret;
	...
	case AF_STATE_NORMAL_INIT:
		 ...
		 * state below */
		if (event == AF_EVT_INTERNAL_STATE_ENTER) {
			/* Check if any new network is saved in PSM.
			   If so, the current network must be unloaded
			   and further in AF_STATE_NORMAL_CONNECTING state the
			   new network configuration will be loaded */
			if (app_check_current_network_dirty()) {
				app_unload_configured_network();
				/* Here, the dirty flag should be unset since
				   the old network(i.e dirty) has been unloaded
				*/
				app_unset_current_network_dirty();
			}
	...
	case AF_STATE_NORMAL_CONNECTING:
		...
		case AF_EVT_NORMAL_CONNECT_FAILED:
			...
			if ((data->app_data).fr == AUTH_FAILED) {
				conn_state.auth_failed++;
			} else if ((data->app_data).fr == DHCP_FAILED) {
			...
			ret =  app_ctrl_event_to_prj(event,
						     &((data->app_data).fr));
			if (sta_stop_cmd == true) {
				sta_stop_cmd = false;
				app_ctrl_notify_event(
		...
		case AF_EVT_NW_SET:
		{
			app_ctrl_prov_network_set(&(data->app_data.
						    current_net));
			...
			memset(&conn_state, 0, sizeof(conn_state));
			app_ctrl_event_to_prj(AF_EVT_NW_SET,
					      &((data->app_data).current_net));
			event_handled = 1;
			break;
		}
	...
	case AF_STATE_UNCONFIGURED:
		...
		case AF_EVT_NW_SET:
			 ...
			 */
			app_load_this_network(&(data->app_data).current_net);
			((data->app_data).current_net).channel = 0;
			/* Write network */
			app_ctrl_prov_network_set(&(data->app_data).
						  current_net);
			/* Jump to normal mode */
...
static void app_ctrl_event_handler(os_thread_arg_t arg)
{
	...
	while (1) {
		...
		if (msg.event == AF_EVT_PS_ENTER ||
			...
			app_ctrl_event_to_prj(msg.event, (void *)
					      (msg.data.app_data.ps_data));
		...
		if (msg.event == AF_EVT_UAP_CLIENT_ASSOC ||
			msg.event == AF_EVT_UAP_CLIENT_DISSOC) {
			app_ctrl_event_to_prj(msg.event, (void *)
				(msg.data.app_data.uap_client_mac_address));
			/* Since these events are not
			 * dependent on state continue
			 * to receive next message
			 */
...
static int __app_ctrl_notify_event(app_ctrl_event_t event,
				   app_ctrl_data_t *data, int flag)
{
	int ret;
	app_ctrl_msg_t msg;
...
{
	int ret;
	ret = app_ctrl_event_to_prj(AF_EVT_NORMAL_LINK_LOST, NULL);
	if (ret == WM_SUCCESS) {
		s.st_next = AF_STATE_NORMAL_CONNECTING;
	} else {
		s.st_next = AF_STATE_NORMAL_DISCONNECTED;
			      ...
			      app_conn_failure_reason_t *reason, int *attempts)
{
	*state = conn_state.state;
	if (conn_state.auth_failed) {
		*reason = AUTH_FAILED;
		*attempts = conn_state.auth_failed;
	} else if (conn_state.dhcp_failed) {
		*reason = DHCP_FAILED;
		*attempts = conn_state.dhcp_failed;
	} else if (conn_state.conn_failed) {
		*reason = NETWORK_NOT_FOUND;
		*attempts = conn_state.conn_failed;
	} else {
		*reason = -1;
	}
}
