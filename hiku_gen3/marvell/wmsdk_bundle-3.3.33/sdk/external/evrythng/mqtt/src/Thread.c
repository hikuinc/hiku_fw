...
#endif
{
	...
	FUNC_ENTRY;
#if defined(CONFIG_OS_FREERTOS)
	if (xTaskCreate((pdTASK_CODE)fn, (const char *)name,stack_size, parameter, (unsigned portBASE_TYPE)priority, &thread) != pdPASS)
		thread = 0;
#elif defined(WIN32) || defined(WIN64)
...
#if defined(CONFIG_OS_FREERTOS)
	Thread_start(secondary, NULL, configMINIMAL_STACK_SIZE * 10, 1, (void*)sem));
#else
