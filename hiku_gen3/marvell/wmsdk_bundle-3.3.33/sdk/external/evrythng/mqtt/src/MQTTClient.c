...
#if defined(CONFIG_OS_FREERTOS)
#define MQTT_CLIENT_RUN_THREAD_NAME	((const signed char *)"MQTTClientRun")
#define MQTT_CONN_LOST_THREAD_NAME	((const signed char *)"MQTTConnLost")
	...
	START_TIME_TYPE start, long millisecsTimeout)
{
	...
	{
#if defined(CONFIG_OS_FREERTOS)
		Thread_start(MQTTClient_run, MQTT_CLIENT_RUN_THREAD_NAME, configMINIMAL_STACK_SIZE * 5, 1, handle);
#else
...
int MQTTClient_connectURI(MQTTClient handle, MQTTClient_connectOptions* options, const char* serverURI)
{
	...
	if (options->ssl->keyStore)
        ...
        m->c->cfg->flags |= TLS_USE_CLIENT_CERT;
        }
	if (options->ssl->privateKey)
        {
...
#if defined(CONFIG_OS_FREERTOS)
		Thread_start(connectionLost_call, MQTT_CONN_LOST_THREAD_NAME, configMINIMAL_STACK_SIZE * 10, 1, m);
#else
