...
#endif
/**
 * mdev AES driver header file is included here as we need make 'aes_t'
 * visible to all CyaSSL files using AES.
 */
#ifdef ENABLE_MCU_AES_HW_ACCL
#include <mdev_aes.h>
#endif /* ENABLE_MCU_AES_HW_ACCL */
...
/**
 * This enum is also defined in mdev_aes.h. Hence, compile out here when
 * hardware AES is used.
 */
#ifndef ENABLE_MCU_AES_HW_ACCL
enum {
...
#ifdef ENABLE_MCU_AES_HW_ACCL
/**
 * The context structure 'aes_t' is defined in mdev_aes.h. When hardware
 * accl is enabled 'Aes' should be union of sw and hw defintion of Aes.
 * This allows us to use the same library binary for both sw and hw accelerated
 * version, without the fear of memory overwrite.
 */
typedef union Aes {
	aes_t aes_hw;
	aes_sw_t aes_sw;
...
/* AES-CTR */
#ifdef WOLFSSL_AES_COUNTER
/* wmsdk: Added for setting CTR key */
int wc_AesSetKeyCtr(Aes* aes, const byte* userKey, word32 keylen,
			const byte* iv, int dir);
