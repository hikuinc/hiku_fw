...
$(t_secboot): sdk/tools/bin/$(os_dir)/secureboot$(file_ext)

sdk/tools/bin/$(os_dir)/secureboot$(file_ext):
	$(AT)$(MAKE) -s -C sdk/tools/host-tools/secureboot OPENSSL_INSTALL_DIR=$(OPENSSL_INSTALL_DIR)
	$(AT)$(t_cp) -a sdk/tools/host-tools/secureboot/$(notdir $@) $@

##################### Secure boot configuration file and make helper
$(sec_conf): $(t_secconf)
	$(AT)$(t_python) $(t_secconf) -d $(sec_conf_dir) -c $(SECURE_BOOT) -s sdk/tools/host-tools/secureboot/sample_keys
	@echo " [sec][conf] $@"

$(sec_type_mk): $(sec_conf) $(ks_hdr) $(t_secboot)
	$(AT)$(t_python) $(t_secboot) -c $(sec_conf) -s $(ks_hdr) -M $(sec_type_mk) $(secboot_flags)
...
define create_secure_prog
  ...
  $($(1)-output-dir-y)/$(1)$(sec_mcufw_type).bin: $($(1)-output-dir-y)/$(1).axf $$(t_axf2fw) $$(sec_conf) $$(ks_hdr) $$(t_secboot)
