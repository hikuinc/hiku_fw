...
define defconfig_cmd
  $(if $(wildcard $(1)),$(eval conf-file:=$(realpath $(1))),$(eval conf-file:=$(b-sdkconfig-dir-y)/$(1)))
...
sdk/src/incl/autoconf.h: .config
	$(oldconfig_cmd)
...
$(t_kconf):
	$(AT)$(MAKE) -s -C sdk/tools/host-tools/kconfig CC=$(HOST_CC) TARGET=$(notdir $@) file_ext=$(file_ext)
	$(AT)$(t_cp) -a sdk/tools/host-tools/kconfig/$(notdir $@) $@
	$(AT)$(MAKE) -s -C sdk/tools/host-tools/kconfig CC=$(HOST_CC) TARGET=$(notdir $@) file_ext=$(file_ext) clean

$(t_mconf):
	$(AT)$(MAKE) -s -C sdk/tools/host-tools/kconfig CC=$(HOST_CC) TARGET=$(notdir $@) file_ext=$(file_ext) all
	$(AT)$(t_cp) -a sdk/tools/host-tools/kconfig/$(notdir $@) $@
	$(AT)$(t_cp) -a sdk/tools/host-tools/kconfig/lxdialog/lxdialog $(dir $@)
	$(AT)$(MAKE) -s -C sdk/tools/host-tools/kconfig CC=$(HOST_CC) TARGET=$(notdir $@) file_ext=$(file_ext) clean

$(t_axf2fw):
	$(AT)$(MAKE) -s -C sdk/tools/host-tools/axf2firmware CC=$(HOST_CC) SDK_PATH=$(CURDIR) TARGET=$(notdir $@)
	$(AT)$(t_cp) -a sdk/tools/host-tools/axf2firmware/$(notdir $@) $@
