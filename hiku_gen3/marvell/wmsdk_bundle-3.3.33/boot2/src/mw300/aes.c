...
#if defined(SEMIHOST_DEBUG) || defined(UART_DEBUG)
...
#define CHECK_AES_INIT_DONE						\
	if (!aes_init_done) {						\
		dbg("AES init not done. Please do it first.\n\r"); \
		return -WM_FAIL;					\
	}
				     ...
				     int dir, AES_Config_Type *aesConfig)
{
	int i;
	switch (dir) {
	case M_AES_ENCRYPTION:
		ADEBUG("Encrypt\n\r");
		aesConfig->encDecSel = AES_MODE_ENCRYPTION;
		break;
	case M_AES_DECRYPTION:
		ADEBUG("Decrypt\n\r");
...
void AesCTRIncrementIV(aes_t *aes, uint32_t len)
{
	...
	for (block_cnt = 0; block_cnt < len; block_cnt++) {
		for (row = 0; row < M_AES_BLOCK_SIZE/4; row--) {
			for (i = 3; i >= 0; i--) {
				index = (row * 4) + i;
				if (++IV[index])
					break;
			  ...
			  uint8_t *outdata, uint32_t len)
{
	...
	if ((aes->mode == AES_MODE_CTR) && (len & (M_AES_BLOCK_SIZE - 1))) {
		 ...
		 * M_AES_BLOCK_SIZE */
		block_data_len = (len / M_AES_BLOCK_SIZE) + 1;
		block_data_len *= M_AES_BLOCK_SIZE;
	}
