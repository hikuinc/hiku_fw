...
gAudioBufferBlob <- blob();
gAudioReadPointer <- gAudioBufferBlob.tell();
...
class AudioUploader
{
   ...
   // static sleepSecs = 0.004;  // For debouncing
    ...
    function startUpload()
    {
        ...
        if (gAudioReadPointer < gAudioBufferBlob.tell())
        {
            local buffer = gAudioBufferBlob[gAudioReadPointer];
    ...
    {
        // This is where we send all the remaining audio to agent, whether it be the last
        // buffer or everything since there wasn't any connection before
        // Send the last chunk to the server, if there is one
    
        currReadPtr = gAudioBufferBlob.tell();
        
        if (currReadPtr < gAudioReadPointer)
        {
            local buffer = gAudioBufferBlob[gAudioReadPointer];
...
function samplerCallback(buffer, length)
{
    ...
    else 
    {
        ...
        gAudioBufferBlob.writeblob(buffer);
        
        if( gAudioChunkCount == 0 )
        {
            // Very first time the sampler is called
            gAudioUploader.startAudio();
        }
